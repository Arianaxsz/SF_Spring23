...
# .35 increase in the variable passengers 
...
train.lm.e <- tslm(train.ts ~ trend, lambda = 0) #to model an exponential trend, we set the lambda argument to zero (a value of 1 is linear) it will transfor our data 
...
svs.train <- window(svs.ts, start = c(1995, 1), end = c(2001, 12))
autoplot(svs.train)
```
...
```
```{r}
#train.lm.e <- tslm(train.ts ~ trend, lambda = 0)
BoxCox.lambda(svs.train)
```
...
svs.train.ARIMA.pred <- svs.train %>% 
  auto.arima(lambda = 0.015) %>% 
  forecast(h = 12)

checkresiduals(svs.train.ARIMA.pred)
```



```{r}
accuracy(svs.train.ARIMA.pred, svs.train)

plot(svs.train.ARIMA.pred, ylim = c(1300, 2600), ylab = "Ridership", xlab = "Time", 
     bty = "l", xaxt = "n", xlim = c(1991, 2006.25), main = "", flty = 2)
axis(1, at = seq(1991, 2006, 1), labels = format(seq(1991, 2006, 1)))
lines(svs.train.ARIMA.pred$fitted, lwd = 2, col = "blue")
lines(window(amtrak.ts, start = c(1991, length(svs.train) - 35), end = c(1991, length(svs.train))))
```
