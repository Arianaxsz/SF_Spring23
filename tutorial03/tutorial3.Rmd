...
- *Centered moving averages* are used to visualise trends, because the averaging suppresses the effects of seasonality and noise in the data. 
...
# the forecast package contains a function, ma(), for calculating centered moving averages
...
## Advanced Exponential Smoothing *are theses errors or are these trends?* 
...
as.ts <- ts(as[,-c(1,3,4)], start = c(1985,1), frequency = 4)
print(as.ts)

autoplot(as.ts) +
  labs(title = "Quarterly shipment  ") +
  ylab(" Shipments 📦 ") + xlab ("Years")
```
...
```{r}
asts.subset <- window(as.ts, start = 1985, end = c(1989,4))
autoplot(asts.subset) +
  autolayer(meanf(asts.subset, h=11), PI=FALSE, series="Mean") +
  autolayer(naive(asts.subset, h=11), PI=FALSE, series="Naïve") +
  autolayer(snaive(asts.subset, h=11), PI=FALSE, series="Seasonal") + 
  ggtitle("Forecasts for quarterly shipment") +
  xlab("Year") + ylab("Shipment") +
...
# the zoo package contains a function, rollmean(), for calculating a trailing moving average
as.trailing <- rollmean(as.ts,
                        k = 12, # this is the window argument
                        align = "right" # this sets to trailing average
                        )
# we can plot these using autoplot() and autolayer(), which is less verbose than base plot
autoplot(as.ts) +
  autolayer(as.centred) +
  autolayer(as.trailing) 
